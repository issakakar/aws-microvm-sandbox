...
# =============================================================================
# microvm-bench image — variant: SCI
#   CPython 3.13 + numpy + matplotlib + pandas + seaborn (dataframe viz)
...
#       -f images/Dockerfile.sci -t microvm-bench-sci ./microvm
#
...
# -----------------------------------------------------------------------------
# Stage 2: Python builder — standalone CPython 3.13 via uv + the full sci stack.
# -----------------------------------------------------------------------------
...
WORKDIR /app
# binutils -> strip(1) for the .so shake-down in shrink.sh.
...
# shrink.sh installs numpy+matplotlib+pandas+seaborn (prebuilt arm64 wheels only),
# then runs compileall -O2/unchecked-hash, prunes tests/docs/stubs/sources, keeps
# only the Agg backend + DejaVu Sans, strips every .so (big OpenBLAS/LAPACK win),
# drops pandas bundled tests, and prints du -sh before/after.
COPY images/shrink.sh /usr/local/bin/shrink.sh
RUN chmod +x /usr/local/bin/shrink.sh && MVB_VARIANT=sci /usr/local/bin/shrink.sh
